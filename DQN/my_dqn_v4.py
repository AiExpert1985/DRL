...
def calculate_loss(agent, lag_agent, use_ddqn=False, gamma=0.99):
...
def train(env, agent, optimizer, config):
    ...
    tb_title = f"-MyDQNv4_{config['id']}_lag={config['use_lag_agent']}_stgy={config['act_strategy']}" \
...
if __name__ == "__main__":
    ...
    game_config["agent_load_score"] = -5
